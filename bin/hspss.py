...
app = Flask(__name__, static_folder=os.getenv('SPRINGHSPSS_STATICDIR').rstrip("/"))

# This is the path to the upload directory
#app.config['UPLOAD_FOLDER'] = sys.argv[3]
app.config['UPLOAD_FOLDER'] = os.getenv('SPRINGHSPSS_UPLOAD') 
...
#    if len(sys.argv)==5:
#TH        app.run(host=sys.argv[1], port=sys.argv[2], threaded=True)
        app.run(host=os.getenv('SPRINGHSPSS_HOST'), port=os.getenv('SPRINGHSPSS_PORT'), threaded=True)
#    else:
