...
def plot_power_spectra(layerline_bessel_pairs, im_theo, im_theo_coll, nyquist, nyquist_simulation, im=None, im_coll=None):
	...
	plot1 = figure(title="Theoretical", x_range=[-nyquist_simulation, nyquist_simulation], y_range=[-nyquist_simulation, nyquist_simulation], 
		plot_width=size, plot_height=size, tools=TOOLS, toolbar_location="above")
	plot1.image([im_theo], x=[-nyquist_simulation], y=[-nyquist_simulation], dw=[2*nyquist_simulation], dh=[2*nyquist_simulation], palette=bokehpalette_1)
	...
	if im!= None:
		...
		plot2_collapse = figure(title="Uploaded", plot_width=size/4, plot_height=size, y_range=plot1.y_range, tools='')
	...
	if im != None:
		plot2.yaxis.axis_label = '1/Angstrom'
...
#''')
	...
	# text_input1 = TextInput(value="1.350", title="Pixelsize in Angstrom:")
	# text_input2 = TextInput(value="1.408, 22.03", title='Rise [in Angstrom], Rotation [in Degrees]')
	# slid = Slider(start=50, end=400, value=150, step=1,title="Helix Width")
	# button = Button(label="Plot", button_type="success")
	# wids = widgetbox(text_input1, text_input2, slid, button, width=250, height=250)
	...
	if im!=None:
		...
		l = gridplot([[plot1, plot1_collapse]], sizing_mode=sizing)
